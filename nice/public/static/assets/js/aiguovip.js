/*
 *  Document   : aiguovip.js
 */
var AAiguovip = function () {
	/**
	 * 处理ajax方式的post提交
	 */
			...
			if ("submit" === self.attr("type") || ajax_url) {
				...
				if ("FORM" === form.get(0).nodeName) {
					...
					if (form.get(0).type === 'checkbox' && form_data === '') {
						AAiguovip.notify('请选择要操作的数据！', 'warning');
						return false;
					}
					// 提交确认
					if (self.hasClass('confirm')) {
						swal({
							...
							if (e.value === true) {
								...
								self.attr("autocomplete", "off").prop("disabled", true);
								// 发送ajax请求-Jun
								$.post(ajax_url, form_data, function (res) {
									pageLoader('hide');
									if (res.code) {
										tips(res.msg, 'success', 'fa fa-check mr-1');
										setTimeout(function () {
											location.href = res.url;
										}, 1500);
									} else {
										swal('很抱歉，发生错误！:)', res.msg, 'error');
										self.attr("autocomplete", "on").prop("disabled", false);
									}
									...
									swal('很抱歉，发生错误！:)', '连接服务器错误！ ', 'error');
									self.attr("autocomplete", "on").prop("disabled", false);
								});
								// 发送ajax请求完毕-Jun

							} else if (e.dismiss === 'cancel' || e.dismiss === 'esc') { 
						...
						return false;
					}else if (self.hasClass('exceldown')) {
				...
				$.post(ajax_url, form_data, function (res) {
					...
					if (res == undefined) {
						...
						if (res.code) {					
							if (res.data) {	
								$('#addsnlist').modal('hide');
								(async function getText() {
									const { value: text } = await Swal.fire({
										title: res.msg,
										type: 'success',
										input: 'textarea',
										text: '以下系统已经存在，自动为您编辑',
										inputPlaceholder: res.data,//提示
										inputValue: res.data,//输入
										confirmButtonText: '导出重复数据',
										cancelButtonText: '取消',
										showCancelButton: true
									})								
									if (text) {							
							...
							swal(res.msg, '', 'error');
							//tips(res.msg, 'warning', 'fa fa-exclamation mr-1');
							self.attr("autocomplete", "on").prop("disabled", false);
						}
					}
				}).fail(function () {
	...
	var iconSearchLoader = function () {
		...
		jQuery('.js-icon-search').on('keyup', function () {
			...
			if ($searchValue.length > 2) { // If more than 2 characters, search the icons
				$searchItems.hide();
				jQuery('code', $searchItems).each(function () {
